...
@st.cache_data
def load_data():
    population_data = pd.read_csv("data/2023_pop.csv", encoding="utf-8")
    with open("data/gdf_korea_sido_2023.json", encoding="utf-8") as f:
        geojson = json.load(f)
    return population_data, geojson

@st.cache_data
def load_movie_data():
    movies = pd.read_csv("data/movies.csv", encoding="utf-8", skiprows=5)
